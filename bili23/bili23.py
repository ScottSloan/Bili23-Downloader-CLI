...
@click.option("--t", "--thread", default = Config.Download.max_thread, help = "指定下载所用线程数，默认为 4 个线程")
@click.option("--q", "--quality", default = Config.Download.quality, help = "指定下载的清晰度，默认为 80 (1080P)")
@click.option("--c", "--codec", default = Config.Download.codec, help = "指定下载的视频编码，默认为 hevc (HEVC/H.265)")
